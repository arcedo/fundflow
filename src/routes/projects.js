...
router.post('/', verifyUserLogged, async (req, res) => {
    try {
        ...
        const [checkUrlRows, checkUrlFields] = await db.getPromise().query('SELECT id FROM projects WHERE url LIKE ?', [`${url}%`]);
...
router.get('/admin/panel', verifyAdminRole, validateQueryParams, async (req, res) => {
    ...
    try {
        const rows = await executeQuery('SELECT p.id, p.title, p.url, u.username FROM projects p JOIN users u ON p.idUser = u.id LIMIT ?, ?', [req.startIndex, req.limit]);
        if (rows.length > 0) {
            res.status(200).json(rows);
        } else {
            res.status(404).send({ message: 'No projects found' });
